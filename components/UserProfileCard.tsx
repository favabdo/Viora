...
export default function UserProfileCard({
  ...
  function roleClass(role: AccessRole | "owner") {
    if (role === "owner" || role === "admin") return "bg-[#EA580C]/12 text-[#EA580C]";
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/70 backdrop-blur-[2px] flex items-center justify-center p-4 z-50 fade-in" onClick={onClose}>
      ...
      >
        <div className="relative bg-[#EA580C]/10 px-6 pt-7 pb-5">
          <button
            type="button"
            onClick={onClose}
                ...
                {isSelf ? (
                  <>
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-lg bg-[#EA580C] hover:bg-[#9A3412] text-white text-xs font-semibold px-3 py-2"
                    >
            ...
            return (
              <button
                ...
                className={`relative shrink-0 px-3 py-2.5 text-[13px] font-medium ${
                  active ? "text-[#EA580C]" : "text-inkFaint hover:text-inkSoft"
                }`}
              >
                {item.label}
          ...
          {loading || !profile ? null : tab === "overview" ? (
            ...
            <div>
                ...
                {(
                  ...
                  <button
                    ...
                    className={`rounded-full px-3 py-1 text-[11px] font-semibold ${
                      taskFilter === id ? "bg-[#EA580C] text-white" : "bg-paperDark text-inkSoft hover:text-ink"
                    }`}
                  >
                    {label}
                  </button>
              ...
              {visibleTasks.length > 5 && !showAllTasks && (
                <button type="button" onClick={() => setShowAllTasks(true)} className="mt-3 text-xs font-semibold text-[#EA580C] hover:underline">
