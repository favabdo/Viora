    ...
    <div className="upgrade-stage px-4 sm:px-6 md:px-10 pt-8 md:pt-12 pb-20">
      <div className="relative mx-auto max-w-[1280px]">
        <div className="flex justify-end mb-4">
          <div className="inline-flex items-center gap-2 rounded-full border border-line bg-surface/70 p-1 backdrop-blur-md">
            <button
              type="button"
              onClick={() => setBilling("monthly")}
              className={`rounded-full px-4 py-1.5 text-xs font-semibold transition-all ${
                !yearly ? "bg-[#7C5CFF] text-white shadow-[0_0_18px_rgba(124,92,255,0.55)]" : "text-inkSoft hover:text-ink"
              }`}
            >
        ...
        <div className="text-center max-w-3xl mx-auto mb-10">
          ...
          <div className="mt-5 flex flex-wrap justify-center gap-2">
            {tags.map((tag) => (
              <span key={tag} className="upgrade-pill rounded-full px-3.5 py-1.5 text-[11px] font-medium text-inkSoft">
                {tag}
              </span>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.08fr)_minmax(0,1fr)_minmax(250px,280px)] items-stretch">
          ...
          <aside className="upgrade-card rounded-2xl p-4 flex flex-col min-h-[440px]">
            ...
            <blockquote className="mt-4 rounded-xl border border-line bg-surfaceSunken/80 p-3">
            ...
            <table className="w-full min-w-[680px] text-sm">
              <thead>
                <tr className="text-inkFaint">
                  ...
                  <th className="text-start font-medium px-4 py-3 text-[#7c5cff]">{t("upgrade.pro.name")}</th>
                  <th className="text-start font-medium px-4 py-3 text-[#0d9488]">{t("upgrade.team.name")}</th>
                </tr>
              </thead>
              <tbody>
                {compareRows.map(({ key, icon: Icon }, i) => (
                  <tr key={key} className={i % 2 === 0 ? "bg-tealSoft/40" : "bg-transparent"}>
                    <td className="px-5 py-3 text-inkSoft">
                      <span className="inline-flex items-center gap-2">
                        <Icon size={14} className="text-[#7c5cff]" />
...
function PlanCard({
  ...
  const ctaClass =
    ctaStyle === "primary"
      ...
      : ctaStyle === "teal"
        ? "border border-cyan-500/60 text-cyan-700 dark:text-[#5eead4] shadow-[0_0_18px_rgba(45,212,191,0.18)] hover:bg-cyan-500/10"
        : "border border-line text-ink hover:bg-tealSoft";
    ...
    <div className={`relative ${featured ? "xl:-mt-3 xl:mb-[-12px]" : ""}`}>
      ...
      >
        ...
        <div className="flex items-start gap-3 mb-5 mt-1">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 leading-tight">
              <span className="text-[1.65rem] font-semibold tracking-tight text-ink">{price}</span>
              <span className="ms-1 text-[12px] text-inkFaint">{unit}</span>
            </p>
            {note && <p className="text-[11px] text-inkFaint mt-0.5">{note}</p>}
          </div>
        </div>
        ...
        <div className={`mt-auto pt-4 rounded-xl p-3 ${accent === "teal" ? "upgrade-ai-box-teal" : "upgrade-ai-box"}`}>
          <p className="text-[11px] font-semibold text-ink mb-2">{aiTitle}</p>
...
function MeetRow({
  ...
  const tones = {
    violet: "bg-teal/15 text-teal shadow-[0_0_12px_rgba(124,92,255,0.25)]",
    blue: "bg-blue-500/15 text-blue-600 dark:text-blue-300 shadow-[0_0_12px_rgba(59,130,246,0.22)]",
    fuchsia: "bg-fuchsia-500/15 text-fuchsia-700 dark:text-fuchsia-300 shadow-[0_0_12px_rgba(217,70,239,0.2)]",
    cyan: "bg-cyan-500/15 text-cyan-700 dark:text-cyan-300 shadow-[0_0_12px_rgba(34,211,238,0.2)]",
  };
  return (
    <div className="flex gap-2.5 rounded-xl border border-line bg-surfaceSunken/50 px-2.5 py-2">
