...
const PROJECT_COLORS = ["#3B82F6", "#3B82F6", "#22C55E", "#F59E0B", "#0891B2", "#10B981"];
const selectClass =
...
function FileGlyph({ file }: { file: LibraryFile }) {
  ...
  if (kind === "video") return <Film size={16} className="text-[#BFDBFE]" />;
  if (kind === "audio") return <Music size={16} className="text-[#6EE7B7]" />;
...
export default function FilesSection({
  ...
  const donut = [
    ...
    { key: "files.documents", value: buckets.doc, color: "#3B82F6" },
  ...
  return (
    <div className="fade-in">
      ...
      <div className="mb-4 grid grid-cols-2 xl:grid-cols-4 gap-3">
        {[
          { title: t("files.totalFiles"), value: String(visible.length), Icon: FileText, color: "#3B82F6" },
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="inline-flex rounded-xl border border-line p-0.5">
                <button
                  ...
                  className={`h-8 w-8 inline-flex items-center justify-center rounded-lg ${view === "list" ? "bg-[#1D4ED8] text-white" : "text-inkSoft"}`}
                >
                  <List size={14} />
                </button>
                <button
                  ...
                  className={`h-8 w-8 inline-flex items-center justify-center rounded-lg ${view === "grid" ? "bg-[#1D4ED8] text-white" : "text-inkSoft"}`}
                >
                  <LayoutGrid size={14} />
                </button>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 p-3">
              {paged.map(({ file, meta }) => (
                <button
                  ...
                  className={`rounded-xl border p-3 text-start ${selectedId === file.id ? "border-[#1D4ED8]" : "border-line hover:border-lineStrong"}`}
                >
                  ...
                  {paged.map(({ file, meta }) => (
                    <tr
                      ...
                      className={`border-b border-line last:border-b-0 cursor-pointer ${
                        selectedId === file.id ? "bg-[#1D4ED8]/10" : "hover:bg-paperDark/50"
                      }`}
                    >
              ...
              <div className="flex items-center gap-1">
                {Array.from({ length: pages }, (_, i) => i + 1).slice(0, 7).map((n) => (
                  <button
                    ...
                    type="button"
                    onClick={() => setPage(n)}
                    className={`h-7 min-w-7 px-1 rounded-lg ${n === pageSafe ? "bg-[#1D4ED8] text-white" : "hover:bg-paperDark text-inkSoft"}`}
                  >
                    {n}
                  </button>
                ))}
            ...
            <div className="rounded-2xl border border-line bg-surface p-4">
              ...
              {!isFolder(selected) && (
                <button
                  ...
                  className="w-full rounded-xl border border-line bg-paperDark/50 p-3 text-start hover:border-[#1D4ED8] mb-3"
                >
            ...
            <>
              <div className="rounded-2xl border border-line bg-surface p-4">
                ...
                <div className="mt-3 h-1.5 rounded-full bg-paperDark overflow-hidden">
                  <div className="h-full rounded-full bg-[#1D4ED8]" style={{ width: `${usedPct}%` }} />
                </div>
                ...
                {(["recent", "shared", "favorites", "trash"] as QuickFilter[]).map((id) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 rounded-xl px-2.5 py-2 text-sm ${
                      quick === id ? "bg-[#1D4ED8]/15 text-ink" : "text-inkSoft hover:bg-paperDark"
                    }`}
                  >
            ...
            <div className="grid grid-cols-3 gap-2">
              {(["free", "project", "task"] as Scope[]).map((id) => (
                <button
                  ...
                  className={`rounded-xl border px-2 py-2 text-xs ${scope === id ? "border-[#2563EB] text-ink" : "border-line text-inkSoft"}`}
                >
                  {t(`files.scope.${id}`)}
                </button>
              ))}
